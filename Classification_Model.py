...
classes = df['Sous Famille'].unique()
...
if __name__ == "__main__":
    wcg = WordCloudGenerator(target='Famille', col='description commande', df=df)
...
tp = TextProcessor()
df = df.assign(Processed_Title=df["description commande"].apply(tp.process))
df.head()
...
if __name__ == "__main__":
    wcg = WordCloudGenerator(target='Famille', col='Processed_Title', df=df)
    wcg.set_classes(classes)
    wcg.generate_wordclouds()
...
model_w2v = Word2Vec(corpus, vector_size=50, min_count=7)
...
if __name__ == "__main__":
    ...
    model.fit(X_train, Y_train_categorical, batch_size=32, epochs=36, validation_split=0.1)
